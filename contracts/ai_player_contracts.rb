...
module AIPlayerContracts
  ...
  def pre_strategic_move(opponent_data, grid)
    check_grid(grid)
    
    assert(opponent_data!=nil, "opponent data cannot be nil")
    assert(opponent_data.respond_to?("keys"))
    assert(opponent_data.respond_to?("[]"))  

    data = opponent_data.keys
